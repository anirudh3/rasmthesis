...
fts = pd.concat([f1, f2], axis=1)
fts = pd.concat([fts, f3], axis=1)

fts = fts.loc[fts['feature'].isin(tracks_sm['Unnamed: 0'])]

# ipd.display(fs)

overall_fts = fts.values[:, 1:]

print "data prepared for clustering", overall_fts.shape
...
#     all_features = numpy.random.rand(A, F)
    model = KMeans(n_clusters=num_clusters, init='k-means++', algorithm='auto')
    model.fit(data)
    return model

model = get_bog_model(overall_fts, 100) # going for 100 clusters
